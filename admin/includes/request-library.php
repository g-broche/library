...
function updateUserStatus($dbco, $ID, $newStatus)
{
    if ($newStatus == 0 || $newStatus == 1 || $newStatus == 2) {
        try {
            $sql = "UPDATE tblreaders SET `Status`=:newStatus WHERE id=:id ";
            $dbco->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
            $statement = $dbco->prepare($sql);
            $statement->bindParam(':id', $ID, PDO::PARAM_INT);
...
function selectAdmin($dbco, $adminName)
{
    try {
        $sqlSelect = "SELECT Password, id FROM admin WHERE FullName=:aName";
        $dbco->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
        $statement = $dbco->prepare($sqlSelect);
        $statement->bindParam(':aName', $adminName);
        $statement->execute();
        $result =  $statement->fetchAll(PDO::FETCH_ASSOC);
        if (count($result) == 1) {
            return [1, $result[0]];
...
function updateAdminPassword($dbco, $id, $pass)
{
    try {
        $sqlUpdate = "UPDATE `admin` SET `Password`=:password WHERE id=:adminId";
        $dbco->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
